...
public class TransactionResource {
    ...
    public Uni<RestResponse<TransactionResponse>> getSingle(Long id) {
        return TransactionDTO.findById(id)
                .onItem().ifNotNull().transformToUni(transaction -> Mutiny.fetch(((TransactionDTO) transaction).tags)
                        .map(tags -> RestResponse.ok(new TransactionResponse((TransactionDTO) transaction))))
